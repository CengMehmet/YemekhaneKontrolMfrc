...
namespace Keyboard
{
    public static class Helper
    {
        ...
        public static void Veritabaniguncelle()
        {
            ...
            {
                conn.Open();
                try { SqlCommand k = new SqlCommand("ALTER TABLE program2 ADD araogunbaslamasaati time(7),araogunbitissaati time(7)", conn); k.ExecuteNonQuery(); } catch (Exception) { }
                try { SqlCommand k = new SqlCommand("ALTER TABLE ogrenci ADD kalangiris int", conn); k.ExecuteNonQuery(); } catch (Exception) { }
                try { SqlCommand k = new SqlCommand("ALTER TABLE ogrenci ADD ogrencitip nvarchar(25)", conn); k.ExecuteNonQuery(); } catch (Exception) { }
            }
        }
        ...
        public static Image ResimYukle(string ogrencino)
        {
            ...
            try
            {
                ...
                {
                    using (FileStream stream = new FileStream("sbl.png", FileMode.Open))
                    {
                        image1 = Image.FromStream(stream);
                        stream.Flush();
                        stream.Close();
